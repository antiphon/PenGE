...
#' @param ranges1 Intra-type ranges, p matrices in a list
#' @param ranges2 Inter-type ranges, p*(p-1)/2 matrices in a list
...
#' [todo]
#'
#' Ranges is now a 2xK matrix giving start of step and end of step on first and second rows, respectively.
#'
#' @return
...
#'
#'
make_Q_stepper_multi_rmat <- function(x, # data
  ...
  for(i in 1:p) {
    ...
    if(auto_sat){
      if(sat1l_given) sats <- sat1l[[i]]
        ...
        dr2 <- apply(ranges1[[i]], 2, diff)^2 #diff(c(0, ranges1[[i]])^2)
    ...
    for(i in 1:(p-1)) {
      ...
      for(j in (i+1):p){
        ...
        if(!auto_sat){
          ...
          xk <- stepper_components(xij, to = NULL, r=ranges2[[k]], sat = sats, bbox = bbox, cross=TRUE, ...)
          dk <- stepper_components(xij, to =  dij, r=ranges2[[k]], sat = sats, bbox = bbox, cross=TRUE, ...)
          ...
          if(sat2l_given){
            ...
            dr2 <- apply(ranges2[[k]], 2, diff)^2 #diff(c(0, ranges2[[k]])^2)
          ...
          xk_ij <- stepper_components(xij, to = NULL, r=ranges2[[k]], sat = sats_ij, bbox = bbox, cross=TRUE, ...)
          xk_ji <- if(sats_same) xk_ij else stepper_components(xij, to = NULL, r=ranges2[[k]], sat = sats_ji, bbox = bbox, cross=TRUE, ...)
          # dummies, same story
          dk_ij <- stepper_components(xij, dij, r=ranges2[[k]], sat = sats_ij, bbox = bbox, cross=TRUE, ...)
          dk_ji <- if(sats_same) dk_ij else stepper_components(xij, dij, r=ranges2[[k]], sat = sats_ji, bbox = bbox, cross=TRUE, ...)
          # collect right bits
