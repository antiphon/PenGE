...
## Introduction
This document provides demonstrates the use of the R-package `PenGE` for estimating significant interactions of a multivariate, piece-wise constant saturation Gibbs model using Group Lasso automatic variable selection. 
...
Not that many. We can increase `nx` but at the cost of introducing correlation in the CV estimates. It remains an open question how bad, if any, such correlation affects the estimation.

Now we need to fit the model for each different quadrat subset. Current implementation uses `mclapply` from `parallel` to run the fitting locally parallel. This will take a moment (adjust `mc.cores` to suit your resources):
...
```{r, cache=TRUE}
Rest <- residuals_cv(cvfit, x = lansing, mc.cores = 5)
```
...
Note that the covariates need to be given explicitly to `residuals_cv`.
...
Confidence intervals of these estimates are not available.
